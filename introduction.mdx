...
description: "Handshake keeps your AI coding sessions in sync across every coding agent you use."
---
...
Handshake runs as a background daemon. It registers itself with each supported agent using the MCP protocol, which means agents treat it as a tool they can call - no manual export, no copy-pasting. Every time you checkpoint a session, Handshake reads the conversation directly from the agent's native session storage, captures the current git state of your working directory, and stores everything locally in `~/.handshake/sessions.db`.
...
Handshake integrates natively with the following:
