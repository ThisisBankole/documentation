...
<Steps>
  ...
  <Step title="Start a session in any agent">
    Open any supported agent; Claude Code, OpenCode, Hermes, or Codex and start working as you normally would. Handshake syncs the session automatically in the background every time the agent produces a response. There is nothing to run and no prompt to send.
