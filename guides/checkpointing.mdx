...
- You can browse your sessions in the [TUI](/guides/using-the-tui) with `handshake browse` and pick one up later.
...
<Note>
  Sessions that predate Handshake — or that ended while the daemon was down — aren't lost either: `handshake pull` imports them straight from each agent's native storage, and the daemon runs the same sweep automatically on startup. See [`handshake pull`](/reference/cli#handshake-pull).
</Note>
