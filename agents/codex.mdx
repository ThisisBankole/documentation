...
<Note>
  **Using the ChatGPT desktop app?** You don't need the `codex` CLI on your PATH. Handshake detects Codex through the `~/.codex` directory or the ChatGPT/Codex app bundle, and the desktop app, CLI, and IDE extension all share the same configuration — so registration works for all of them at once. If your Codex home lives elsewhere, set `CODEX_HOME` before running setup.
</Note>
